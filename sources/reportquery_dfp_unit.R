reportquery_dfp_unit <- function(year,m_start,m_end,day_start,day_end,device_type,dfp_unit,only_total){
  ...
  #  where ((ad_exchange_gfp_inventory_unit_name in ('4507451 » LEAD_HP', '4507451 » SUPERLEAD_HP', '4507451 » SUPERLEAD_HP_middle', '4507451 » SQR_HP') and ad_exchange_platform_type_name in ('Desktop', 'Tablets')) and not (ad_exchange_dfp_ad_unit_top_level like 'TXTLINK_HP'))
    request_data <- list(reportJob=list(reportQuery=list(dimensions='AD_UNIT_NAME',
                                                       ...
                                                       statement=list(query=paste0("WHERE AD_UNIT_NAME = ","'",dfp_unit,"'", " AND DEVICE_CATEGORY_ID IN",device_type))  
  ...
  {   
    a=tail(report_dat,1)
    a[1]=paste(dfp_unit,device_type_data,toString(day_start),".",toString(day_end),toString(m_start),".",toString(year))
    a[2]="Total"  
