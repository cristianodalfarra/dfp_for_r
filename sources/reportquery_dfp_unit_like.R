reportquery_dfp_unit_like <- function(year,m_start,m_end,day_start,day_end,device_type,dfp_unit,only_total){
  ...
  request_data <- list(reportJob=list(reportQuery=list(dimensions='AD_UNIT_NAME',
                                                       ...
                                                       statement=list(query=paste0("WHERE AD_UNIT_NAME LIKE '",dfp_unit,"' ", " AND DEVICE_CATEGORY_ID IN",device_type))  
  ...
  #library(xlsx)
  a[1]=paste(gsub('%', '', dfp_unit),device_type_data,toString(day_start),"-",toString(day_end),toString(m_start),"/",toString(year))
